...
interface INFTImplementationETH {
    function initialize(string memory name_, string memory symbol_, address _curve, address _factory, uint256 _m, uint256 _c, uint256 _feeMultiplier, uint256 _dividendFeeMultiplier, uint256 _nftType, string memory _baseTokenURI) external;
    function changeURI(string memory _newURI) external;
    function balanceOf(address owner) external view returns (uint256);
}

contract NFTImplementationETH is ERC721AEnumerable, OwnableUpgradeable {
    ...
    function initialize(string memory name_, string memory symbol_, address _curve, address _factory, uint256 _m, uint256 _c, uint256 _feeMultiplier, uint256 _dividendFeeMultiplier, uint256 _nftType, string memory _baseTokenURI) external initializerERC721A {
    ...
    function buy(uint256 quantity) external payable {
        ...
        _transferFee(factory.getProtocolFeeReceiver(), protocolFee);
        _transferFee(owner(), tradeFee);
    ...
    function sell(uint256 quantity) external {
        ...
        if (factory.isHasDiscountNFT(msg.sender)) {
            _transferFee(msg.sender, outputValue + protocolFee);
        } else {
            _transferFee(factory.getProtocolFeeReceiver(), protocolFee);
            _transferFee(msg.sender, outputValue);
        }
    ...
    function _refundTokenToSender(uint256 inputAmount) internal {
        if (msg.value > inputAmount) {
            (bool success, ) = payable(msg.sender).call{ value: msg.value - inputAmount }('');
    ...
    function _distributeEthers(uint256 dividendFee, address[] memory holders) internal {
        ...
        for (uint256 i = 0; i < getHoldersCount(); ++i) {
            (bool success, ) = payable(holders[i]).call{ value: dividend * ERC721AUpgradeable.balanceOf(holders[i]) }('');
            require(success, 'disperse ether failed');
        }
